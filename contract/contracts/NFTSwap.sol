...
contract NFTSwap {
    ...
    modifier needsExistentProposal(uint256 proposalId) {
        require(
            proposals[proposalId].nftAddress != IERC721(address(0x0)),
    ...
    modifier needsProposalOwnership(uint256 proposalId) {
        require(
            ...
            "Specified proposal is not yours"
        );
        _;
    }
    ...
    modifier needsExistentBid(uint256 bidId) {
        require(
            bids[bidId].nftAddress != IERC721(address(0x0)),
    ...
    modifier needsBidOfProposal(uint256 proposalId, uint256 bidId) {
        require(
            bids[bidId].proposalRef == proposalId,
            "Specified bid was not made for specified proposal"
        );
        _;
    }
    ...
    function makeProposal(IERC721 nftAddress, uint256 tokenId)
        external
        needsNFT(nftAddress, tokenId)
    {
    ...
    function makeBid(
        ...
        external
        needsNFT(bidNftAddress, bidNftTokenId)
    ...
    function acceptBid(uint256 proposalId, uint256 bidId)
        external
        needsExistentProposal(proposalId)
        needsProposalOwnership(proposalId)
        needsExistentBid(bidId)
        needsBidOfProposal(proposalId, bidId)
    {
        ...
        if (nftAddress2.ownerOf(tokenId2) != bidderAddress) {
            deleteBid_(bidId);
            revert(
                "The bidder does not own the nft anymore. Bid has been deleted."
            );
        }
        if (nftAddress2.getApproved(tokenId2) != address(this)) {
            deleteBid_(bidId);
            revert(
                "The bidder removed approval for the bidded nft to this contract. Impossible to complete the swap until the bidder re-approves the nft. Bid has been deleted."
            );
        }
    ...
    function refuseBid(uint256 proposalId, uint256 bidId)
        external
        needsExistentProposal(proposalId)
        needsProposalOwnership(proposalId)
        needsExistentBid(bidId)
    ...
    function deleteBid(uint256 bidId)
        external
        needsExistentBid(bidId)
        needsBidOwnership(bidId)
    {
        delete proposals[bids[bidId].proposalRef].bidsRef[
            bids[bidId].indexInProposal
        ];
        delete bids[bidId];
        bidsGaps.push(bidId);
    }
    ...
    {
        require(
            proposals[proposalId].bidsRef.length > index,
            "Specified bid does not exist"
        );
